...
log_attack() {
    # Log to current directory (no permission issues)
...
# Enhanced CVE Database with new exploits
...
# New: Port Scanning Function
scan_ports() {
    local target=$1
    log_attack "Scanning ports on $target"
    
    # Common web ports + suspicious ports
...
# New: Data Exfiltration Function
exfiltrate_data() {
    local target=$1
    ...
    # Try database dumps if common services found
...
log_attack() {
echo "[$(date +'%H:%M:%S')] $1" | tee -a ediop3_ops.log
echo "$(date -Iseconds) | $1" >> .ediop3_hist  # Now logs to current directory
}
